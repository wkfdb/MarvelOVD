...
__all__ = ["SSODFastRCNNOutputLayers"]


class SSODFastRCNNOutputLayers(FastRCNNOutputLayers):
    """
    ...
    def __init__(
        ...
        novel = [4,5,11,12,15,16,21,23,27,29,32,34,45,47,54,58,63]
        base = [i for i in range(self.num_classes) if i not in novel]
        self.base = torch.Tensor(base).long().cuda()
        self.novel = torch.Tensor(novel).long().cuda()
    ...
    def losses(self, predictions, proposals):
        ...
        gt_is_base = (
            cat([p.gt_is_base for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        )
        ...
        with torch.no_grad():
            ...
            cls_weights = gt_confidence.clone()
            # cls_weights = gt_confidence.new_ones(gt_confidence.size())
            cls_weights[gt_classes==self.num_classes]=1
           

        losses = {
            # "loss_cls": cross_entropy(scores, gt_classes, reduction="mean"),
            # "loss_cls": (self.loss_fn(scores, gt_classes) * gt_confidence).mean(),
            "loss_cls": (self.loss_fn(scores, gt_classes) * cls_weights).mean(),
            "loss_box_reg": self.box_reg_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes, gt_is_base
            ),
        }
        return {k: v * self.loss_weight.get(k, 1.0) for k, v in losses.items()}

    def box_reg_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, gt_is_base):
        """
        Args:
        ...
        # fg_inds = nonzero_tuple((gt_classes >= 0) & (gt_classes < self.num_classes) & (gt_is_base>0))[0]
