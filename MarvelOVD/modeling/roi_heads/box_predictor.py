...
class VLPLMFastRCNNOutputLayers(FastRCNNOutputLayers):
    ...
    def losses(self, predictions, proposals):
        ...
        with torch.no_grad():
            ...
            novel_scores = softmax_scores[:,self.novel].sum(-1)
            gt_mask = gt_confidence>=1
            new_confidence = gt_confidence + novel_scores
            new_confidence[gt_mask] = 1
            cls_weights = new_confidence
    ...
    def box_reg_loss(self, proposal_boxes, gt_boxes, pred_deltas, gt_classes, gt_confidence):
        ...
        fg_inds = nonzero_tuple((gt_classes >= 0) & (gt_classes < self.num_classes) & (gt_confidence>=1))[0]
